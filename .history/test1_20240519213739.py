    ...
    def _set_values(self, paras_values):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for key, item in jobs.items():
                # executor.submit(set_value_swat, os.path.join(self.work_path, key), list(item.keys()), item)
                futures.append(executor.submit(set_value_swat, os.path.join(self.work_path, key), list(item.keys()), item))
            for future in futures:
                future.result()  # 这将阻塞，直到对应的任务完成
        a=1    
