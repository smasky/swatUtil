...
// Helper function to extract a substring and trim it
...
// Convert a substring to a double, treating empty as 0
...
void readFormattedData(const std::string& line, std::vector<double>& data) {
    // Define the field widths based on Fortran format: 1x,i2,1x,i2,5x,i4,1x,i2, ...
    const std::vector<size_t> fieldWidths = {
        1, 2, 1, 2, 5, 4, 1, 2, 1, 4, 1, 3, 1, 6, 1, 12, 1, 6, 1, 11, 1, 8, 1, 6, 1, 16 * 5
    };

    size_t pos = 0; // 当前解析的位置

    for (size_t width : fieldWidths) {
        // 提取字段并解析
        std::string field = extractField(line, pos, width);
        if (width > 1) {
            field.erase(0, field.find_first_not_of(' ')); // 去除前导空格
        }
        data.push_back(parseField(field));
        pos += width; // 移动位置到下一个字段起始
    }
}
