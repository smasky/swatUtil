from my_module import _set_value_swat
import os
import re
import os
import itertools
...
class SWAT_UQ():
    ...
    n_output=1
    def __init__(self, work_path, paras_file_name, observe_file_name, swat_exe_name, max_workers=12):
        self.work_path=work_path
    ...
    def _recond_default_values(self):
        ...
        if not os.path.exists(default_database_path):
            ...
            for i in range(self.n_input):
                ...
                try:
                    type, suffix=self.paras_files.loc[para_name, ["type", "file_name"]]
            ...
            for key, value in tasks.items():
                if key in self.hru_suffix:
                    for code in self.hru_list:
                        jobs.append((self.work_path, code+"."+key, value["para_list"]))
                    ...
                    jobs.append((self.work_path, "basin.bsn", value["para_list"]))
            
            
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # 提交任务到线程池
                futures = [executor.submit(_read_value_swat, *job) for job in jobs]

            for future in futures:
                res=future.result()
                for key, items in res.items():
                    values=' '.join(str(value) for value in items)
                    name, file_name=key.strip('_')
                        # read_value_swat(os.path.join(self.work_path, code+"."+key), value["para_list"])
    ...
    def _initial(self):
        ...
        self.paras_file=pd.read_excel(os.path.join(self.work_path, 'SWAT_paras_files.xlsx'))
        
    
file_path="D:\SiHuRiver\model\FuTIanSi001\Scenarios\Test\TxtInOut"
file_name="000020001.sol"      
swat_cup=SWAT_UQ(work_path=file_path,
                    para_file_name="SWAT_paras_files.xlsx",
                    observe_file_name="observed.txt",
                    swat_exe_path="swat_64rel.exe",
                    max_workers=12)        
