    ...
    def _record_default_values(self):
        ...
        with open(var_infos_path, 'r') as f:
            ...
            for line in lines:
                ...
                mode.append(tmp_list[2])
                types.append(tmp_list[1])
                low_bound.append(float(tmp_list[3]))
                up_bound.append(float(tmp_list[4]))
                range.append(tmp_list[5:])
    ...
    def _initial(self):
        ...
        if self.special_paras_file is not None:
            with open(os.path.join(self.work_path, self.special_paras_file), 'r') as f:
                ...
                for line in lines:
                    tmp_list=line.split()
                    self.paras_file.loc[tmp_list[0]]=tmp_list[2:]
        print(self.paras_file)      
