            ...
            for key, value in tasks.items():
                if key in self.hru_suffix:
                    for code in self.hru_list:
                        jobs.append((os.path.join(self.work_path, code+"."+key), value["para_list"]))
                    ...
                    jobs.append((os.path.join(self.work_path, "basin.bsn"), value["para_list"]))
            
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # 提交任务到线程池
                futures = [executor.submit(read_value_swat, *jobs[i]) for i in range(len(jobs))]
