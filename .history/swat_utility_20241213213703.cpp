...
void _write_value(const std::string file_path, const std::string file_name, const std::vector<std::string> var_list, std::map<std::string, std::string> default_value, 
    ...
    }else if (file_extension=="ops")
    {
        ...
        for (int i = 0; i < var_list.size(); ++i) {
            ...
            std::ostringstream oss;
            oss<< std::right;
            oss << std::setw(3) << static_cast<int>(number[0]) // 1x, i2 (2字符宽度)
              << std::setw(3) << static_cast<int>(number[1])  // 1x, i2
              << std::setw(9) << static_cast<int>(number[2]) // 5x, i4
              << std::setw(3) << static_cast<int>(number[3])  // 1x, i2
              << std::setw(5) << static_cast<int>(number[4]) // 1x, i4
              << std::setw(4) << static_cast<int>(number[5]) // 1x, i3
