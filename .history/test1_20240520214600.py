from my_module import read_value_swat, set_value_swat, read_simulation
import os
import re
import itertools
import pandas as pd
...
class SWAT_UQ():
    ...
    def _subprocess(self, work_path, input_x, id):
        ...
        simulation_data=np.array(read_simulation(os.path.join(work_path, "output.rch"), 6, 10))
        rch_flows=simulation_data[self.rch_id-1::self.rch_id]
        # rch_flows=simulation_data.query('RCH=={}'.format(self.rch_id))['FLOW_OUTcms'].to_numpy()
        y=-r2_score(self.observed_data, rch_flows[self.begin_calibration-self.output_skip_days-1:self.end_calibration-self.output_skip_days])
        # except:
            # y=-np.inf 
        return (id, y)
    def _get_simulation_data(self, work_path):
