...
from UQPyL.surrogates import LinearRegression as LR
...
X=np.loadtxt('./res_X_500.txt', dtype=np.float64)
Y=np.loadtxt('./res_Y_500.txt', dtype=np.float64)
...
pr=LR(scalers=(MinMaxScaler(0,10), MinMaxScaler(0,10)), loss_type='Lasso', alpha=0.11)
pr.fit(X, Y)
