...
std::vector<double>& read_value_swat(const std::string file_path, const std::vector<std::string> &varname_list)
{
...
PYBIND11_MODULE(my_module, m) {
    m.doc() = "Swat plugin"; // 可选的模块文档字符串
    m.def("read_value_swat", &read_value_swat, "A function that reads and processes file data based on regex patterns.");
}