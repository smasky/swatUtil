...
class SWAT_UQ():
    ...
    n_output=1
    def __init__(self, work_path, paras_file_name, observed_file_name, swat_exe_name, max_workers=12):
        self.work_path=work_path
    ...
    def _set_values(self, paras_values):
        ...
        for key, item in self.tasks.items():
            ...
            for para in para_list:
                ...
                mode=paras_values(self.dict_index_paras[para][1])
                if mode=='v':
                    ...
                    if key=='sol':
                        ...
                        str_value=" ".join(str(numbers[i]))
                    else:
                        str_value='{:.2f}'.format(float(str_origin_value)*(1+value))
                elif mode=='a':
                    str_origin_value=self.default_values.loc[(self.default_values['para_name'] == para) & (self.default_values['file_name'] == key), 'value'].values[0]
                    if key=='sol':
                        numbers=str_origin_value.split()
                        ...
                        str_value='{:.2f}'.format(float(str_origin_value)+value)
                tasks[key][para]=str_value
        
        jobs=[]
        for key, items in tasks.items():
            
            if key in self.watershed_list:
                for sub in self.Watershed[key]:
                    file_name=sub+"."+key
                    para_list=items.keys()
                    jobs.append((os.path.join(self.work_path, file_name), para_list, items))
            elif key in self.hru_list:
                for sub in self.hru_list:
                    file_name=sub+"."+key
                    para_list=items.keys()
                    jobs.append((os.path.join(self.work_path, file_name), para_list, items))
            elif key in ['bsn']:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交任务到线程池
            futures = [executor.submit(set_value_swat, *job) for job in jobs]
    ...
    def _initial(self):
        ...
        self.simulation_days=(self.end_date-self.begin_date).days+1
        self.output_skip_years=int(dict_values["NYSKIP"][0])
