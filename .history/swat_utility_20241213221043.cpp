...
void readFormattedData(const std::string& line, std::vector<double>& data) {
    // Define the field widths based on Fortran format (i2, i2, i4, i2, i4, i3, f6.2, f12.5, f6.2, f11.5, f8.2, f6.2, 16f5.2)
    const std::vector<size_t> fieldWidths = {
        2, 2, 4, 2, 4, 3, 6, 12, 6, 11, 8, 6 // 将16f5.2按宽度80处理
    };
    ...
    for (size_t width : fieldWidths) {
        ...
        data.push_back(parseField(field)); // 将解析后的数值加入结果
        pos += width; // 移动到下一个字段起始位置
    }
}
