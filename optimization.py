...
def _set_paras(file_path, name, value, mode, origin_value=None):
    ...
    with open(file_path, "r+") as f:
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                if mode=='r':
                    value=float(origin_value)*(1+value)
                elif mode=='a':
                    value=float(origin_value)+value
...
work_temp_dir="D:\SiHuRiver\model\FuTIanSi001\Scenarios\Test\TxtInOut"
swat_cup=SWAT_CUP(work_path=work_temp_dir,
                    para_file_name="paras_infos.txt",
                    observe_file_name="observed.txt",
                    swat_exe_path="swat_64rel.exe",
                    rch_id=40)
X=np.loadtxt('pso.txt', dtype=np.float64)
xx=X[-1, :]
swat_cup.evaluate(xx.reshape(1,-1))
# pso=PSO(problem=swat_cup,maxFEs=30000, maxIterTimes=600)
# res=pso.run()
# a=1
